...
export interface LineSidebarProps {
  ...
  /** Per-item destinations. Supplying one renders that item as a real <a href>,
   *  which restores keyboard activation, modifier-clicks and crawlability. */
  itemHrefs?: (string | undefined)[];
  onItemClick?: (index: number, label: string, event: React.MouseEvent | React.KeyboardEvent) => void;
  className?: string;
}
...
const LineSidebar = ({
  ...
  const handleClick = useCallback(
    (index: number, label: string, event: React.MouseEvent | React.KeyboardEvent) => {
      setActiveIndex(index);
      onItemClick?.(index, label, event);
    },
    [onItemClick]
  );

  // Anchors handle Enter natively; this only covers items rendered without an
  // href, which fall back to role="button" and therefore need Enter and Space.
  const handleKeyDown = useCallback(
    (index: number, label: string, event: React.KeyboardEvent) => {
      if (event.key !== 'Enter' && event.key !== ' ') return;
      event.preventDefault();
      handleClick(index, label, event);
    },
    [handleClick]
  );
  ...
  useEffect(
    () => () => {
      ...
      // Must null the ref, not just cancel: startLoop() guards on it being
      // null, and StrictMode's mount/cleanup/mount would otherwise leave a
      // dead frame id here and wedge the loop shut permanently.
      rafRef.current = null;
    },
    []
  );
    ...
    >
      ...
      >
        {items.map((label, index) => {
          ...
          const labelClass =
            'relative inline-flex items-baseline leading-[1.2] [color:color-mix(in_srgb,var(--accent-color)_calc(var(--effect,0)*100%),var(--text-color))] [font-size:var(--font-size)] [transform:translateX(calc(var(--effect,0)*var(--max-shift)))] focus-visible:outline focus-visible:outline-1 focus-visible:outline-offset-4 focus-visible:[outline-color:var(--accent-color)]';
          const labelContent = (
            <>
              {showIndex && (
          ...
          return (
            <li
              ...
              onClick={e => handleClick(index, label, e)}
            ...
            >
              ...
              {href ? (
                <a
                  ...
                  // stopPropagation keeps the <li> handler from firing a second
                  // time; the <li> still covers the padded hit area around it.
                  onClick={e => {
                    e.stopPropagation();
                    handleClick(index, label, e);
                  }}
                ...
                <span
                  role="button"
                  tabIndex={0}
                  onKeyDown={e => handleKeyDown(index, label, e)}
                  className={labelClass}
                >
                  {labelContent}
                </span>
